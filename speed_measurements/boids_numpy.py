...
def main(boids_number, iterations):
    ...
    for i in range(boids_number):
        x_pos = np.random.uniform(x_pos_range[0], x_pos_range[1])
        y_pos = np.random.uniform(y_pos_range[0], y_pos_range[1])
        x_vel = np.random.uniform(x_vel_range[0], x_vel_range[1])
        y_vel = np.random.uniform(y_vel_range[0], y_vel_range[1])
    ...
    for i in range(iterations):
        update_boids(boids)
    print("--- NumPy: Elapsed in %s seconds, for %d Boids and %d iterations ---" % (time.time() - start_time, boids_number, iterations))
